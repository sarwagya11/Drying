...
CSV (``phase1_full_results.csv``) in the ``phase1_out`` directory.
"""
...
def rolling_mean_series(times: np.ndarray, values: np.ndarray, window: int = 21) -> Tuple[np.ndarray, np.ndarray]:
    if times.size == 0 or values.size == 0:
        return np.array([]), np.array([])

    df = pd.DataFrame({"t": times, "res": values})
    df_sorted = df.sort_values("t")
    rolling = df_sorted["res"].rolling(window=window, min_periods=max(3, window // 3), center=True)
    mean = rolling.mean().to_numpy()
    return df_sorted["t"].to_numpy(), mean
...
def create_plot(
    ...
    gs = GridSpec(2, 1, height_ratios=[3, 2], hspace=0.08)
    ...
    t_grid = np.linspace(0, float(np.max(t_norm)) if t_norm.size else 0.0, 400)
    ...
    for model_key, result in results.items():
        ...
        pred_grid = config.func(t_eff_grid, *result.params)
        ax_curve.plot(t_grid, pred_grid, label=result.model_key, color=color, linewidth=2)

        t_eff = effective_time(t_norm, result.tau)
        predictions = config.func(t_eff, *result.params)
        residuals = mr_raw - predictions
        ax_residual.scatter(
            t_norm,
            residuals,
            ...
            alpha=0.35,
            edgecolors="none",
            label=f"{result.model_key} residual",
        )

        rm_t, rm_val = rolling_mean_series(t_norm, residuals)
        if rm_t.size:
            ax_residual.plot(
                rm_t,
                rm_val,
                color=color,
                linewidth=2.0,
                label=f"{result.model_key} roll(21)",
            )

    ax_residual.axhline(0, color="gray", linestyle="--", linewidth=1)

    ax_curve.tick_params(labelbottom=False)
    ax_curve.set_ylabel("MR")
    ax_curve.set_title(csv_path.stem)
    ax_curve.legend()

    ax_residual.set_xlabel("time (min)")
    ax_residual.set_ylabel("Residual (MR)")
    ax_residual.legend(loc="upper right", fontsize="small", ncol=1)
    ax_residual.set_title("Residuals (scatter) with rolling mean")
...
def process_file(csv_path: Path) -> Dict[str, object]:
    ...
    meta = df.iloc[0]
    best_model_name = best_result.model_key if math.isfinite(best_result.aicc) else "None"
    ...
    for key, config in MODELS.items():
        ...
        params_default = {name: np.nan for name in config.param_names}
        params_default.update(zip(config.param_names, result.params))

        for param_name, value in params_default.items():
            summary[f"{prefix}_{param_name}"] = value if math.isfinite(value) else np.nan

        summary[f"{prefix}_tau"] = result.tau if math.isfinite(result.tau) else np.nan
        summary[f"{prefix}_RMSE"] = result.rmse if math.isfinite(result.rmse) else np.nan
        summary[f"{prefix}_AICc"] = result.aicc if math.isfinite(result.aicc) else np.nan

    summary["best_tau"] = best_result.tau if math.isfinite(best_result.tau) else np.nan
...
def main() -> None:
    ...
    summary_path = OUTPUT_DIR / "phase1_full_results.csv"
